...
import { ReonoASTParser, type RouteInfo as ASTRouteInfo } from "./ast-parser.js";
...
export function reonoClient(options: ReonoClientOptions): Plugin {
  ...
  async function analyzeServerFile(filePath: string): Promise<RouteInfo[]> {
    ...
    return astRoutes.map((astRoute): RouteInfo => ({
  ...
  function generateClientCode(
    ...
    const routeMethods = Array.from(methodGroups.entries())
      .map(([method, methodRoutes]) => generateMethodOverloads(method, methodRoutes))
      .join("\n");
...
${pathTypes || "  // No paths with parameters"}
  ...
  function generateMethodOverloads(method: string, routes: RouteInfo[]): string {
    const overloads = routes.map(route => {
      const paramType = route.params.length > 0
  ...
  function generateMethodImplementation(method: string, routes: RouteInfo[]): string {
    const implementations = routes.map(route => {
