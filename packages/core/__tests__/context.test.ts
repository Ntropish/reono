...
import { buildContext, parseCookies, getMimeTypeFromFilename } from '../src/runtime/pipeline';
...
describe('Utility Functions', () => {
  it('parseCookies handles various cookie formats', () => {
    expect(parseCookies('a=1; b=2; c=3')).toEqual(new Map([
      ['a', '1'], ['b', '2'], ['c', '3']
    ]));
    
    expect(parseCookies('session=abc=def; empty=')).toEqual(new Map([
      ['session', 'abc=def'], ['empty', '']
    ]));
