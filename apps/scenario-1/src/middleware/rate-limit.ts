...
export function createRateLimit(config: RateLimitConfig): MiddlewareHandler {
  return async (c, next) => {
    const now = Date.now();
    const key = config.keyGenerator ? config.keyGenerator(c) : c.req.headers.get('x-forwarded-for') || 'global';
      ...
      return new Response(
        ...
        }),
        {
          ...
          headers: {
            ...
            'X-RateLimit-Reset': Math.ceil((limit.windowStart + config.windowMs) / 1000).toString(),
          },
        }
      );
    }
    ...
    if (response instanceof Response) {
      ...
      response.headers.set('X-RateLimit-Remaining', Math.max(0, config.maxRequests - limit.requests).toString());
      response.headers.set('X-RateLimit-Reset', Math.ceil((limit.windowStart + config.windowMs) / 1000).toString());
    }
