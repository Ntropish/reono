...
export function reonoClient(options: ReonoClientOptions): Plugin {
  ...
  const clientCode = generateClientCode(routes, clientName, baseUrl, outputDirPath);
  ...
  const { typeImports, aliasMap, needsZodInfer } = generateTypeImports(routes, outputDirPath);
...
${generateRouteTypesRecord(routes, aliasMap)}
...
${implementations}
  ...
  function generateRouteTypesRecord(routes: RouteInfo[], aliasMap: Map<string, string>): string {
    // Group routes by HTTP method
    const methodGroups = new Map<string, RouteInfo[]>();
    routes.forEach((route) => {
      const method = route.method.toUpperCase();
        ...
        const routeEntries = methodRoutes
          .map((route) => {
            ...
            if (!route.hasBody) {
              ...
              const v = route.validation?.body as any;
              if (v && typeof v === 'object' && v.kind === 'schemaRef') {
                const alias = aliasMap.get(v.importPath);
                if (alias) {
                  bodyType = v.partial
                    ? `Partial<ZodInfer<typeof ${alias}.${v.name}>>`
                    : `ZodInfer<typeof ${alias}.${v.name}>`;
                } else {
                  bodyType = "any";
                }
              } else {
                bodyType = typeof v === 'string' ? v : "any";
              }
            }
...
${methodInterfaces}
  ...
  function generateTypeImports(routes: RouteInfo[], outputDirPath: string) {
    ...
    const schemaRefs = bodySchemaRefs.filter((v) => typeof v === 'object' && v.kind === 'schemaRef') as Array<{ importPath: string; name: string }>;
    const uniqueByPath = new Map<string, { importPath: string; alias: string }>();
    let counter = 0;
    ...
    const typeImports = Array.from(uniqueByPath.values()).map(({ importPath, alias }) => `import type * as ${alias} from '${importPath}';`);
    const aliasMap = new Map<string, string>(Array.from(uniqueByPath.entries()).map(([abs, v]) => [abs, v.alias]));
    const needsZodInfer = schemaRefs.length > 0;
    return { typeImports, aliasMap, needsZodInfer };
  }
}
