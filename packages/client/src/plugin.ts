...
${implementations}
  ...
  function generateRouteTypesRecord(
    routes: RouteInfo[],
...
${methodInterfaces}
  ...
  function generateTypeImports(routes: RouteInfo[], outputDirPath: string) {
    ...
    const schemaRefs = bodySchemaRefs.filter(
      (v) => typeof v === "object" && v.kind === "schemaRef"
    ) as Array<{ importPath: string; name: string }>;
    const uniqueByPath = new Map<
      string,
      { importPath: string; alias: string }
    >();
    let counter = 0;
    ...
    const typeImports = Array.from(uniqueByPath.values()).map(
      ({ importPath, alias }) =>
        `import type * as ${alias} from '${importPath}';`
    );
    const aliasMap = new Map<string, string>(
      Array.from(uniqueByPath.entries()).map(([abs, v]) => [abs, v.alias])
    );
